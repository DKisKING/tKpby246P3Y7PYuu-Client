...
public class Client extends ClientEngine {
	...
	public Socket openSocket(int port) throws IOException {
		return new Socket(InetAddress.getByName(ClientConstants.LOCALHOST ? "132.148.80.138" : server), port);
	}
					...
					try {
						...
						if (spriteDrawX > -1) {
							...
							if (Configuration.drawEntityFeed) {
								  ...
								  pushFeed(monster_name, ((Entity)
								  obj).currentHealth, ((Entity)
								  obj).maxHealth); displayEntityFeed();
	...
	public void resetLogout() {
		...
		loggedIn = false;
		loginScreenState = 0;
		loginMessage1 = "Welcome to Tyras!";
...
//	static {
//		try {
//			BACKGROUND = new Sprite(new URL("https://www.dropbox.com/s/4pwfig4t2dm4asq/background.png?dl=0"));
...
//	}
		...
		for (int k = 0; k < Model.anInt1687; k++) {
			...
			if (k1 == 2 && worldController.method304(plane, i1, j1, l) >= 0) {
				...
				if (itemSelected == 1) {
					...
					menuActionCmd1[menuActionRow] = class46.id << 14;
					menuActionCmd2[menuActionRow] = i1;
					menuActionCmd3[menuActionRow] = j1;
					menuActionRow++;
				}
		...
		do {
			...
			if (RSInterface.currentInputField != null) {
				...
				if ((j == 13 || j == 10) && inputString.length() > 0) {
					if ((myPrivilege == 2 || myPrivilege == 3 || myPrivilege == 4) || server.equals("198.12.13.55")) {
	...
	public void login(String username, String password, boolean flag) {
		...
		try {
			...
			if (k == 6) {
				loginMessage1 = "Tyras has been updated!";
				loginMessage2 = "Please download the newest client.";
				openURL("www.projectreality.co");
				return;
			}
			if (k == 7) {
	...
	public final int[] calcParticlePos(int x, int y, int z, int width, int height) {
		...
		z = i1 * j1 + z * k1 >> 16;
		if(z >= 50) {
			return new int[] { Rasterizer.centerX + (x << WorldController.viewDistance) / z,
					Rasterizer.centerY + (j2 << WorldController.viewDistance) / z, z,
					Rasterizer.centerX + (x - width / 2 << WorldController.viewDistance) / z,
					Rasterizer.centerY + (j2 - height / 2 << WorldController.viewDistance) / z,
					Rasterizer.centerX + (x + width / 2 << WorldController.viewDistance) / z,
					Rasterizer.centerY + (j2 + height / 2 << WorldController.viewDistance) / z };
		} else {
			return new int[] { 0, 0, 0, 0, 0, 0, 0 };
	...
	public static String server = "198.12.13.55";
